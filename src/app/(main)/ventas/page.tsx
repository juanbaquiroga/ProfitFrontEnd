"use client"

import { VentasTable } from "@/components/shared/VentasTable";
import { useVentas } from "@/hooks/useVentas";

export default function Ventas() {
    const { data, isLoading, isError } = useVentas();

    return (
        <div className="flex h-full flex-row w-full overflow-hidden bg-[#FAFBFA]">
            <div className="flex w-full flex-col gap-4 p-8 h-full max-w-[1400px] mx-auto overflow-hidden">
                <div className="flex justify-between items-center px-1">
                    <h1 className="text-[22px] font-bold tracking-tight text-slate-900">Historial de Ventas</h1>
                </div>

                {isLoading ? (
                    <div className="flex flex-1 justify-center items-center">
                        <div className="flex flex-col items-center gap-2">
                            <div className="w-10 h-10 border-4 border-profit/20 border-t-profit rounded-full animate-spin"></div>
                            <p className="text-[15px] font-bold text-profit/80">Cargando historial de ventas...</p>
                        </div>
                    </div>
                ) : isError ? (
                    <div className="flex flex-1 justify-center items-center bg-red-50/50 rounded-xl border border-red-100">
                        <p className="text-sm font-medium text-red-500">Hubo un error al cargar las ventas. Por favor, intenta de nuevo más tarde.</p>
                    </div>
                ) : (
                    <VentasTable
                        data={data || []}
                        pagination={true}
                        pageSize={15}
                        showSearch={true}
                    />
                )}
            </div>
        </div>
    );
}
